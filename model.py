...
class Model():
    ...
    def build(self, embedding_file=None):
        ...
        #rnn_inputs = tf.one_hot(self.X, 6010)
        ...
        with tf.variable_scope('rnn'):
            ...
            layers = [tf.nn.rnn_cell.BasicLSTMCell(self.dim_embedding)
            ...
            self.rnn_outputs, self.final_state = tf.nn.dynamic_rnn(cell=cell, inputs=data, initial_state=self.init_state)    

        # concate every time step
        #seq_output = tf.concat(outputs_tensor, 1)

        # flatten it
        seq_output_final = tf.reshape(self.rnn_outputs, [-1, self.dim_embedding])
        ...
        y_reshape = tf.reshape(self.Y, [-1])
        tf.summary.histogram('logits', logits)

        self.predictions = tf.nn.softmax(logits, name='predictions')

        #loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(self.Y, [-1])$
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=y_reshape)
